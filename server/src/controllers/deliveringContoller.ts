...
export const deliverHandler = async (
  ...
  try {
    ...
    const promises = orders.map((order: unknown) =>
      deliver(order, jobId, jobName, pipelineName, 1),
    );

    const responses = await Promise.all(promises);

    const allSuccess = responses.every((r) => r.success);
    responses.forEach((r, _) => {
      if (!r.success) {
        logger.error(`❌ Failed to deliver to ${r.res.subscriberName}`, {
          jobName,
          pipelineName,
          subscriber: r.res.subscriberName,
        });
      } else {
        logger.success(`✅ Successfully delivered to ${r.res.subscriberName}`, {
          jobName,
          pipelineName,
          subscriber: r.res.subscriberName,
        });
    ...
    try {
      ...
      if (jobDetails?.history) {
        const historyTable = jobDetails?.history.map((h: any) => ({
          ...
          Time: h.time
            ...
            : "N/A",
          Duration: h.time
            ? `${Math.round((Date.now() - new Date(h.time).getTime()) / 1000)}s`
            : "Pending",
        }));
      ...
      if (
        ...
        const attemptsTable = jobDetails?.deliveryAttempts.map(
          (attempt: any) => ({
            Subscriber: attempt.subscriber,
            Attempt: attempt.attempt,
            Status: attempt.status,
            Time: attempt.time
              ? new Date(attempt.time).toLocaleTimeString("en-US", {
                  hour12: false,
                  hour: "2-digit",
                  minute: "2-digit",
                  second: "2-digit",
      ...
      logger.info("📊 Delivery process completed", {
        jobName,
        pipelineName,
        success: allSuccess,
      });
